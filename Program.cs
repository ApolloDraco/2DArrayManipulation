...
class Program
{
    static void Main(string[] args)
    {
        ...
        do
        {
            ...
            matrix = RotateLeftDiagonal(matrix, ref rows, ref cols);
            ...
            matrix = RotateRightDiagonal(matrix, ref rows, ref cols);
    ...
    static int[,] RotateLeftDiagonal(int[,] matrix, ref int row, ref int col)
    {
    ...
    static int[,] RotateRightDiagonal(int[,] matrix, ref int row, ref int col)
    {
        int[,] rotatedMatrix = new int[col, row];
        ...
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                rotatedMatrix[col - j - 1, i] = matrix[i, j];
            }
        }
